...
local function chest_is_valid(chest, inventory)
    ...
    for _, item_data in pairs(contents) do
        inventory[item_data.name] = item_data.count
    end
        for index = 1, 40 do
...
local function get_nearby_chests(player, a, furnace, wagon)
    ...
    for _, e in pairs(player.physical_surface.find_entities_filtered({type = container_type, area = area, force = 'player'})) do
    ...
    if #containers <= 0 then
        if is_mod_loaded('Krastorio2') then
            for _, e in pairs(player.physical_surface.find_entities_filtered({type = 'assembling-machine', area = area, force = 'player'})) do
                if ((player.physical_position.x - e.position.x) ^ 2 + (player.physical_position.y - e.position.y) ^ 2) <= r_square then
                    i = i + 1
                    containers[i] = e
                end
    ...
    sort_entities_by_distance(player.physical_position, containers)
...
local function insert_to_furnace(player_inventory, chests, name, count)
    ...
    for _, chest in pairs(chests) do
        ...
        if chest.type == 'assembling-machine' then
            chest_inventory = chest.get_inventory(defines.inventory.crafter_input)
        elseif chest.type == 'mining-drill' then 
            chest_inventory = chest.get_inventory(defines.inventory.fuel)
        ...
        if chest_inventory then
            if (chest.type == 'furnace' or chest.type == 'assembling-machine' or chest.type == 'mining-drill')  then
                if name == 'stone' then
