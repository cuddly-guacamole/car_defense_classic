...
Event.add(defines.events.on_configuration_changed, function()
    rebuild_everyone()
end)

-- 调试命令：开发者热更场景脚本后，手动重建自己的 GUI（需管理员）
Commands.new('reload-ui', '重建当前玩家的所有 GUI（场景脚本热更后用于手动修复界面）')
    :require_admin()
    :callback(function(player)
        Public.rebuild_player(player)
        if player and player.valid then
            player.print('[GUI] 已重建所有界面')
        end
    end)
